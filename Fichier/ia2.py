...
def minimax(state, depth, player, difficulty):
    ...
    # Configuration initiale pour le meilleur mouvement
    if player == COMP:
        best = [-1, -1, -infinity]
    else:
        best = [-1, -1, +infinity]

    # Si le jeu est terminé ou la profondeur maximale est atteinte, retournez le score.
    ...
    for cell in empty_cells(state):
        x, y = cell[0], cell[1]
        state[x][y] = player
        score = minimax(state, depth - 1, -player, difficulty)
        state[x][y] = 0
        score[0], score[1] = x, y
